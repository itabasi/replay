...
//cout << "f1slot: " << f1slot << " ";//endl;
		...
		*fDebugFile<<" int_chn chan data "<<dec<<chn<<"  "<<chan
	     ...
	     //
		    int max_cout=3; //by itabashi
		    
	     if (okslot && f1slot!=30 && ((*loc) & DATA_CHK) != F1_RES_LOCK ) {
	       if(nwarnings<max_cout) {
		      ...
		      if(nwarnings<max_cout) cout << "\tHit-FIFO overflow";
	          }
	          if ( (*loc) & F1_OUT_OFLW ) {
                      fWarnings[chSlot] |= 1UL << 1; // 2nd bit
		      if(nwarnings<max_cout) cout << "\tOutput FIFO overflow";
	          }
	          if ( ! ((*loc) & F1_RES_LOCK ) ) {
                      fWarnings[chSlot] |= 1UL << 2; // 3rd bit
                      if(nwarnings<max_cout) cout << "\tWarning: F1 TDC " << hex << (*loc) << dec << "\tResolution lock failure!"<<"\tnwarnings"<< nwarnings;
	          }
	          if(nwarnings<max_cout) cout <<endl;

...
void TriFadcCherenkov::Clear( Option_t* opt )
{
  ...
  for( Int_t i=0; i<fNelem; ++i ) {
    ...
    fT[i] = fT_c[i] = fA[i] = fA_p[i] = fA_c[i] = -100.0; //change initial num    
    fPeak[i]=0.0;
    fT_FADC[i]=0.0;
    fT_FADC_c[i]=0.0;
  }
	 ...
	 //	 cout<<"data : "<<data<<" ";
      ...
      if(adc){
        ...
        //       if(foverflow[k]+funderflow[k]+fpedq[k] != 0) printf("Bad Quality: (over, under, ped)= (%i,%i,%i)\n",foverflow[k],funderflow[k],fpedq[k]);
	      ...
	      //       cout<<"tempPed "<<tempPed<<" Pulsedata : "<<static_cast<Double_t>(evdata.GetData(kPulsePedestal,d->crate,d->slot,chan,0))<<" kpulsePed "<<kPulsePedestal<<" ";
            }
            else
            {

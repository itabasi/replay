...
// Note, this increments evbuffer
  ...
  int max_debug=10;  //Added by itabashi
		...
		*fDebugFile<<" int_chn chan data "<<dec<<chn<<"  "<<chan
	     ...
	     if (okslot && f1slot!=30 && ((*loc) & DATA_CHK) != F1_RES_LOCK ) {
	       if(nwarnings<max_debug) {
		      ...
		      if(nwarnings<max_debug) cout << "\tHit-FIFO overflow";
	          }
	          if ( (*loc) & F1_OUT_OFLW ) {
                      fWarnings[chSlot] |= 1UL << 1; // 2nd bit
		      if(nwarnings<max_debug) cout << "\tOutput FIFO overflow";
	          }
	          if ( ! ((*loc) & F1_RES_LOCK ) ) {
                      fWarnings[chSlot] |= 1UL << 2; // 3rd bit
                      if(nwarnings<max_debug) cout << "\tWarning: F1 TDC " << hex << (*loc) << dec << "\tResolution lock failure!";
	          }
	          if(nwarnings<max_debug) cout << endl;
